...
<body>
    ...
    <main>
      ...
      <div class="stack">
        ...
        <section>
          <div class="section-head">
            <h2>Observability</h2>
            <span class="subtle" id="obs-session"></span>
          </div>
          <div id="obs-live"></div>
          <div id="obs-subagents"></div>
          <div id="obs-tools"></div>
          <div id="obs-orchestration"></div>
        </section>
        <section>
          <div class="section-head">
            <h2>Actions</h2>
  ...
  <script>
    ...
    function renderObservability(obs) {
      if (!obs) {
        $('#obs-session').textContent = 'no data';
        $('#obs-live').innerHTML = '<div class="empty">Observability snapshot unavailable.</div>';
        $('#obs-subagents').innerHTML = '';
        $('#obs-tools').innerHTML = '';
        $('#obs-orchestration').innerHTML = '';
        return;
      }

      $('#obs-session').textContent = obs.sessionId || 'no active session';
      const live = obs.live || {};
      const loopDetected = obs.loops && obs.loops.detected;
      $('#obs-live').innerHTML = '<div class="metric"><span>Live cost</span><strong>' +
        escapeHtml(live.total_cost_usd != null ? live.total_cost_usd : (live.cost || '-')) + '</strong></div>' +
        '<div class="metric"><span>Tool count</span><strong>' + escapeHtml(live.tool_count || 0) + '</strong></div>' +
        '<div class="metric"><span>Loop warning</span><strong>' + (loopDetected ? '<span class="pill bad">yes</span>' : '<span class="pill good">no</span>') + '</strong></div>';

      const subagents = Array.isArray(obs.subagents) ? obs.subagents.slice(-8).reverse() : [];
      $('#obs-subagents').innerHTML = subagents.length
        ? subagents.map(row => '<div class="work-item"><div class="row"><strong>' + escapeHtml(row.agent || row.worker || '-') +
          '</strong>' + statePill(row.event === 'complete' ? 'done' : 'running') + '</div>' +
          '<div class="subtle">' + escapeHtml(row.ts || '') + (row.duration_ms ? ' - ' + row.duration_ms + 'ms' : '') + '</div></div>').join('')
        : '<div class="empty">No subagent events yet.</div>';

      const tools = Array.isArray(obs.toolActivity) ? obs.toolActivity.slice(-6).reverse() : [];
      $('#obs-tools').innerHTML = tools.length
        ? tools.map(row => '<div class="subtle">' + escapeHtml(row.tool_name || row.tool || 'tool') + ' - ' + escapeHtml(row.ts || '') + '</div>').join('')
        : '<div class="empty">No recent tool activity.</div>';

      const orchSessions = Array.isArray(obs.orchestrationSessions) ? obs.orchestrationSessions : [];
      const gan = obs.gan || {};
      const loops = obs.loopSnapshots || {};
      $('#obs-orchestration').innerHTML =
        '<div class="subtle">Orchestration sessions: ' + escapeHtml(orchSessions.join(', ') || 'none') + '</div>' +
        '<div class="subtle">GAN iteration: ' + escapeHtml(gan.iteration != null ? gan.iteration : '-') +
        ' score: ' + escapeHtml(gan.latestScore != null ? gan.latestScore : '-') + '</div>' +
        '<div class="subtle">Active loops: ' + escapeHtml((loops.active || []).length || 0) + '</div>';
    }
    ...
    async function load() {
      ...
      renderObservability(snapshot.observability);
