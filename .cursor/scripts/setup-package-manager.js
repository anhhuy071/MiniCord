...
function showHelp() {
  console.log(`
Package Manager Setup for Cursor Agent
...
Options:
  ...
  --global <pm>   Set global preference (saves to ~/.cursor/ecc/package-manager.json)
...
function detectAndShow() {
  ...
  console.log(`  Environment var: ${process.env.ECC_PACKAGE_MANAGER || process.env.CLAUDE_PACKAGE_MANAGER || 'not set'}`);
  console.log('');
...
function setGlobal(pmName) {
  ...
  try {
    ...
    console.log('  Saved to: ~/.cursor/ecc/package-manager.json');
    console.log('');
  } catch (err) {
