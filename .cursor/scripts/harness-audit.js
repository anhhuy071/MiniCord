...
function isCursorHarness(rootDir) {
...
function getCursorObservabilityChecks(rootDir) {
  ...
  const hooksJson = safeRead(rootDir, '.cursor/hooks.json');
  const hooksWired = hooksJson.includes('afterShellExecution')
    && hooksJson.includes('subagentStart')
    && fileExists(rootDir, '.cursor/hooks/post-tool-telemetry.js');

  return [
    {
      id: 'cursor-telemetry-hooks',
      category: 'Tool Coverage',
      points: 4,
      scopes: ['repo', 'hooks'],
      path: '.cursor/hooks.json',
      description: 'Cursor hooks wire shell/MCP/file-edit/subagent telemetry',
      pass: hooksWired,
      fix: 'Wire Cursor lifecycle hooks to post-tool-telemetry.js for real-time observability.',
    },
    {
      id: 'cursor-observability-readiness',
      category: 'Quality Gates',
      points: 4,
      scopes: ['repo'],
    ...
    },
    {
      ...
      description: 'Orchestration status and worktree CLIs exist',
      pass: fileExists(rootDir, 'scripts/orchestration-status.js')
        && fileExists(rootDir, 'scripts/orchestrate-worktrees.js'),
      fix: 'Add orchestration-status.js and orchestrate-worktrees.js CLI wrappers.',
    },
    {
      id: 'cursor-eval-runtime',
      category: 'Eval Coverage',
      points: 3,
      scopes: ['repo'],
    ...
    {
      id: 'cursor-observability-tests',
      category: 'Eval Coverage',
      points: 2,
      scopes: ['repo'],
      path: 'tests/',
      description: 'Observability-focused automated tests exist',
      pass: countFiles(rootDir, 'tests', '.test.js') >= 3,
      fix: 'Add tests for cursor-telemetry, observability-readiness, and orchestration helpers.',
    },
  ];
}
...
function getConsumerChecks(rootDir) {
  ...
  return [
    ...
    ...getCursorObservabilityChecks(rootDir),
  ];
}
