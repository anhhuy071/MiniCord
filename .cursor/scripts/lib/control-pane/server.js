...
const { buildObservabilitySnapshot } = require('../observability-snapshot');
const { readJsonlTail } = require('../observability-lib');
...
function createControlPaneServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && requestUrl.pathname === '/api/metrics/live') {
        const sessionId = requestUrl.searchParams.get('session') || null;
        sendJson(res, 200, buildObservabilitySnapshot({ cwd: repoRoot, sessionId }).live || null);
        return;
      }

      if (req.method === 'GET' && requestUrl.pathname === '/api/subagents') {
        const sessionId = requestUrl.searchParams.get('session') || null;
        sendJson(res, 200, {
          events: buildObservabilitySnapshot({ cwd: repoRoot, sessionId }).subagents,
        });
        return;
      }

      if (req.method === 'GET' && requestUrl.pathname === '/api/tools/recent') {
        sendJson(res, 200, { events: readJsonlTail('tool-usage.jsonl', 50) });
        return;
      }

      if (req.method === 'GET' && requestUrl.pathname === '/api/orchestration/sessions') {
        const { listOrchestrationSessions } = require('../orchestration-paths');
        sendJson(res, 200, { sessions: listOrchestrationSessions(repoRoot) });
        return;
      }

      if (req.method === 'GET' && requestUrl.pathname === '/api/orchestration/gan') {
        const { readGanHarnessState } = require('../gan-harness-reader');
        sendJson(res, 200, readGanHarnessState(repoRoot));
        return;
      }

      if (req.method === 'GET' && requestUrl.pathname === '/api/orchestration/loops') {
        const { ingestLoopSnapshots } = require('../loop-snapshot-ingest');
        sendJson(res, 200, ingestLoopSnapshots());
        return;
      }

      const orchestrationWorkerMatch = requestUrl.pathname.match(/^\/api\/orchestration\/([^/]+)\/workers$/);
      if (req.method === 'GET' && orchestrationWorkerMatch) {
        const { collectSessionSnapshot } = require('../orchestration-session');
        sendJson(res, 200, collectSessionSnapshot(decodeURIComponent(orchestrationWorkerMatch[1]), repoRoot));
        return;
      }
